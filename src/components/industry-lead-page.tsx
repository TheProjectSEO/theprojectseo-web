...
function SectionIntro({
  ...
  return (
    <div className={center ? "mx-auto max-w-3xl text-center" : "max-w-3xl"}>
      ...
      {lead ? (
        <Lead className={`mt-6 ${center ? "mx-auto" : ""}`}>{lead}</Lead>
      ) : null}
    </div>
  );
}
      ...
      <Container className="relative">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border py-5 text-xs text-ash">
          <p>
            ...
            <span className="font-semibold text-stone">Aditya Aman</span>,
      ...
      <Container>
        ...
        <div className="mt-14 divide-y divide-border border-y border-border">
          {page.obstacles.map((obstacle, index) => (
            ...
            >
              <span className="font-display text-5xl italic text-accent/70">
                0{index + 1}
              </span>
              <div>
            ...
            <tbody className="divide-y divide-border">
              {page.architecture.map((row) => (
                <tr key={row.pageType} className="align-top">
                  ...
                  <td className="px-6 py-5 text-sm leading-relaxed text-slate">
                    {row.intent}
                  </td>
                  <td className="px-6 py-5 text-sm leading-relaxed text-slate">
                    {row.examples}
    ...
    <section className="bg-cream py-24">
      <Container>
        <SectionIntro
          ...
          lead={page.processLead}
        />
        <div className="mt-14 grid gap-6 md:grid-cols-2">
          {page.process.map((step) => (
            <article
              key={step.number}
              className="border border-border bg-paper p-7 sm:p-8"
            >
      ...
      <Container>
        <div className="grid gap-14 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
          <div>
            ...
            />
            <p className="mt-7 text-sm leading-relaxed text-ash">
              {page.measurementBoundary}
            </p>
          </div>
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <aside className="border border-border bg-cream p-7 sm:p-8">
            ...
            <p className="mt-4 text-sm leading-relaxed text-slate">
              These primary sources inform the technical and content standards
              described on this page. They do not imply endorsement of
              TheProjectSEO.
            </p>
        ...
        <div className="mt-6 grid border-l border-t border-border-strong md:grid-cols-2 lg:grid-cols-4">
          {page.relatedLinks.map((link) => (
            ...
            >
              ...
              <span
                className="mt-5 inline-block text-accent"
                aria-hidden="true"
              >
                →
              </span>
